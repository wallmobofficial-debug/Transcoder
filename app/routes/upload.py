...
Flow:
  ...
  2. Wait for a processing slot (global concurrency cap — see app.main),
     then transcode to HLS with ffmpeg (segments + variant playlist).
    ...
    async with aiofiles.open(input_path, "wb") as out:
        while chunk := await file.read(CHUNK_SIZE):
            ...
            if len(header_buffer) < 64:
                header_buffer += chunk[: 64 - len(header_buffer)]
...
async def _process_video(
    ...
    Waits on the global processing_semaphore first so at most
    `video_processing_concurrency` videos are ever being transcoded at
    once — important headroom on a memory-capped host.
    """
